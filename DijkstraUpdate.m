...
while CurrentRow ~= goal(1) || CurrentCol ~= goal(2)
    [CurrentDistance, CurrentBlock] = min(distance);
    ...
    for i = 1:1:NumberDirect
        ...
        costTerm = CurrentDistance + k_cost*cost(neighborRow, neighborCol);

        if  mapArr(neighborRow, neighborCol) == doneMark || costTerm > CostValueArr(neighborRow, neighborCol)
            continue;
        end

        CostValueArr(neighborRow, neighborCol) = costTerm;
        distance(neighborRow, neighborCol) = CurrentDistance + k_cost*cost(neighborRow, neighborCol);
    ...
    distance(CurrentRow, CurrentCol) = Inf;
